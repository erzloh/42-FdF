...
/*   Updated: 2022/12/23 10:30:52 by eholzer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_grid_lines(t_mlx mlxd, t_map map, t_grid_data g)
{
	...
	p1.x = ORIGIN_X_ISO + (g.i - g.j) * (TILE_WIDTH / 2);
	p1.y = ORIGIN_Y_ISO + (g.i + g.j) * (TILE_HEIGHT / 2);
	p1.y -= (map.tab_2d[g.j][g.i] * g.scaler);
	if (g.i + 1 < map.x_len)
	{
		p2.x = ORIGIN_X_ISO + ((g.i + 1) - g.j) * (TILE_WIDTH / 2);
		p2.y = ORIGIN_Y_ISO + ((g.i + 1) + g.j) * (TILE_HEIGHT / 2);
		p2.y -= (map.tab_2d[g.j][g.i + 1] * g.scaler);
		draw_line(mlxd, p1, p2);
	}
	if (g.j + 1 < map.y_len)
	{
		p2.x = ORIGIN_X_ISO + (g.i - (g.j + 1)) * (TILE_WIDTH / 2);
		p2.y = ORIGIN_Y_ISO + (g.i + (g.j + 1)) * (TILE_HEIGHT / 2);
